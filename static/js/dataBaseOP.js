...
var lanxDB = function(dbname) {
    ...
    return {
        ...
        // 初始化数据库，如果需要则创建表
        init: function(tableName, features) {
            this.switchTable(tableName);
            this.dropTable();
            this._colums = [];
            this._colums.push( {name:'id',type:'integer primary key autoincrement'});
            this._colums.push({name:'name',type:'integer'});
            for(var i = 0; i<features.length;i++)
            {
                this._colums.push({name:'feature'+i ,type:'integer'});
            }
            this._colums.push({name:'clusterType',type:'integer'});
            this._colums.push({name:'finalType',type:'integer'});
            this._colums.push({name:'version',type:'integer'});
            this.createTable(this._colums);
            this._colums.shift();
            return this;
        },
        ...
        insertData: function(data,version, callback) {
            var that = this;
            var sql = "INSERT INTO " + this._table;
            var cols = [], qs = [];
            for (var i in this._colums) {
                cols.push(this._colums[i].name);
                qs.push('?');
            }
            qs.pop();
            sql += " ("+cols.join(',')+") Values ("+qs.join(',')+","+version+")";
